...
rem
rem  Serves the REPO ROOT so both front-ends work (each fetches factory /
...
rem     /wasm/wam/host.html   - the Web Audio Module (WAM) host (worklet engine)
rem  The server runs in its own window; close that window to stop it.
rem
rem  Usage:  run_server.bat [port] [wasm|wam]
rem            port          static-server port          (default 8777)
rem            wasm | wam    which page to open          (default wasm)
...
if /i "%PAGE%"=="wam" ( set "OPENPATH=/wasm/wam/host.html" ) else ( set "OPENPATH=/wasm/web/index.html" )
...
echo   WAM host:  http://localhost:%PORT%/wasm/wam/host.html
echo Opening %URL%
